...
class CKCScrollView: UIView,UIScrollViewDelegate {
    ...
    func changeImage(left : Int , middle : Int , right : Int){
        //给重用的三个imageView附上图片 jp ~~URL
        
        let left_Url = URL.init(string: imageArray![left])
        let mid_Url = URL.init(string: imageArray![middle])
        let right_Url = URL.init(string: imageArray![right])
        
        leftImageView?.kf.setImage(with: left_Url, placeholder: UIImage.init(named:"Me_ProfileBackground"), options:  [.transition(ImageTransition.fade(1))], progressBlock: { (receivedSize, totalSize) in
            
        }) { ( image, error, cacheType, imageURL ) in
            
        }
        middleImageView?.kf.setImage(with: mid_Url, placeholder: UIImage.init(named:"Me_ProfileBackground"), options:  [.transition(ImageTransition.fade(1))], progressBlock: { (receivedSize, totalSize) in
            
        }) { ( image, error, cacheType, imageURL ) in
            
        }
        rightImageView?.kf.setImage(with: right_Url, placeholder: UIImage.init(named:"Me_ProfileBackground"), options:  [.transition(ImageTransition.fade(1))], progressBlock: { (receivedSize, totalSize) in
